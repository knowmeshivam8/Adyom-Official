        ...
        <div className="overflow-hidden">
            ...
            <section className="relative py-20 md:py-28 bg-heritage-terracotta">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
                    <motion.div {...fadeInUp}>
                        <Badge variant="gold" className="mb-4 text-sm px-4 py-1 font-light">✦ Heritage Blog</Badge>
                        <h1 className="text-4xl md:text-5xl font-serif font-bold text-text-main">Stories of Heritage</h1>
                        <p className="mt-4 text-xl font-serif italic text-text-main max-w-2xl mx-auto font-light">
                            Insights, reflections, and stories from the canvas of heritage — where tradition meets transformation
                        </p>
                    </motion.div>
                </div>
                <div className="absolute bottom-0 left-0 right-0">
                    <svg viewBox="0 0 1440 80" className="w-full h-auto"><path fill="#EDD8D0" d="M0,40 C480,80 960,0 1440,40 L1440,80 L0,80 Z" /></svg>
                </div>
            </section>

            {/* Filters */}
            <section className="py-6 bg-heritage-terracottaBg">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-4">
                        <div className="relative flex-1 max-w-md">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2 rounded-md border border-heritage-terracottaDark/30 bg-heritage-terracottaBg text-sm font-light text-heritage-brown placeholder:text-heritage-sand focus:ring-2 focus:ring-heritage-gold focus:outline-none"
                            />
                        </div>
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-full text-xs font-light transition-colors ${
                                        activeCategory === cat.key
                                            ...
                                            : 'bg-heritage-terracottaMid text-text-main hover:bg-heritage-terracottaDark/40'
                                    }`}
                                >
                                    {cat.name}
                                </button>
            ...
            {/* Posts Grid */}
            <section className="py-12 md:py-16 bg-heritage-terracottaBg">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    {loading ? (
                        <PostGridSkeleton items={6} />
                    ) : filtered.length === 0 ? (
                        <div className="text-center py-12">
                            <p className="font-light text-text-main">No posts found. Try a different category or search.</p>
                        </div>
                    ) : (
                                    ...
                                    <Link to={`/blog/${post.slug}`}>
                                        <Card className="group hover:shadow-lg transition-all duration-300 overflow-hidden">
                                            ...
                                            <CardContent className="p-5 space-y-3">
                                                <Badge variant={categoryColors[post.category] || 'secondary'} className="text-xs font-light">{post.category}</Badge>
                                                <h3 className="font-serif font-semibold text-heritage-terracottaDark group-hover:text-heritage-terracottaDark transition-colors line-clamp-2">
                                                    {post.title}
                                                </h3>
                                                <p className="text-sm font-light text-text-main line-clamp-2">{post.excerpt}</p>
                                                <div className="flex items-center justify-between text-xs font-light text-text-main">
            ...
            {/* Newsletter */}
            <section className="py-12 bg-heritage-terracottaMid">
                <div className="max-w-2xl mx-auto px-4 text-center space-y-4">
                    <h2 className="text-2xl font-serif font-bold text-heritage-terracottaDark">Subscribe to Heritage Stories</h2>
                    <p className="font-light text-text-main">Get the latest articles, insights, and heritage inspiration delivered to your inbox.</p>
                    <div className="flex gap-2 max-w-md mx-auto">
                        <input type="email" placeholder="Your email address" className="flex-1 px-4 py-2 rounded-md border border-heritage-terracottaDark/30 bg-heritage-terracottaBg text-sm font-light text-heritage-brown" />
                        <Button variant="gold" size="sm" className="font-medium">Subscribe</Button>
                    </div>
                </div>
            </section>
        </div>
