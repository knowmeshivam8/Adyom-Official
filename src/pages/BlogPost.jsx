        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-heritage-creamLight">
                <div className="text-center space-y-4">
                    <h2 className="text-2xl font-serif font-bold text-heritage-terracottaDark">Post Not Found</h2>
                    <p className="font-light text-text-main">The blog post you're looking for doesn't exist.</p>
                    <Link to="/blog"><Button variant="gold" className="font-medium">Back to Blog</Button></Link>
                </div>
            </div>
        );
    }
            ...
            <section className="relative py-20 md:py-28 bg-heritage-terracotta">
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <motion.div {...fadeInUp} className="space-y-6">
                        <Link to="/blog" className="inline-flex items-center gap-1 text-sm font-light text-text-main hover:text-heritage-gold transition-colors">
                            <ArrowLeft className="w-4 h-4" /> Back to Blog
                        </Link>
                        <Badge variant="gold" className="text-sm px-4 py-1 font-light">{post.category}</Badge>
                        <h1 className="text-3xl md:text-4xl font-serif font-bold text-text-main">{post.title}</h1>
                        <div className="flex flex-wrap gap-4 text-sm font-light text-text-main">
            ...
            <section className="py-12 md:py-16 bg-heritage-creamLight">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    <motion.div {...fadeInUp} className="prose prose-lg max-w-none">
                        <div className="font-light text-heritage-brownLight leading-relaxed space-y-6 [&_h3]:font-serif [&_h3]:text-heritage-terracottaDark [&_h3]:text-xl [&_h3]:font-semibold [&_p]:text-text-main [&_p]:leading-relaxed [&_p]:font-light">
                            {(typeof post.content === 'string' ? post.content : fallbackPost.content).split('\n\n').map((block, i) => {
                                ...
                                if (firstLine.length < 60 && !firstLine.startsWith('At') && !firstLine.startsWith('In') && !firstLine.startsWith('Heritage') && !firstLine.startsWith('What') && !firstLine.startsWith('Perhaps') && lines.length <= 2) {
                                    return <h3 key={i} className="font-serif text-xl font-semibold text-heritage-terracottaDark">{block}</h3>;
                                }
                                return <p key={i} className="font-light text-text-main leading-relaxed">{block}</p>;
                            })}
                        </div>
                    </motion.div>
                    ...
                    <div className="flex items-center gap-4 mt-8">
                        <Button variant="outline" size="sm" onClick={handleLike} disabled={likeSubmitting || !isAuthenticated} className="font-light">
                            <Heart className={`w-4 h-4 mr-1 ${liked ? 'fill-heritage-terracotta text-heritage-terracottaDark' : ''}`} /> Like ({post.likes})
                        </Button>
                        <Button variant="outline" size="sm" className="font-light"><Share2 className="w-4 h-4 mr-1" /> Share</Button>
                    </div>
                    ...
                    <div className="space-y-6">
                        <h3 className="font-serif font-semibold text-heritage-terracottaDark">Comments ({post.comments})</h3>
                        <div className="space-y-4">
                            {(post.commentsList || []).map((c, i) => (
                                <div key={i} className="bg-heritage-cream rounded-lg p-4">
                                    <div className="flex items-center gap-2 mb-2">
                                        <div className="w-8 h-8 rounded-full bg-heritage-terracotta flex items-center justify-center text-heritage-gold font-serif text-sm font-bold">
                                            {c.name?.charAt(0) || 'A'}
                                        </div>
                                        <span className="font-light font-medium text-sm text-heritage-terracottaDark">{c.name}</span>
                                        <span className="text-xs font-light text-text-main">{c.date}</span>
                                    </div>
                                    <p className="font-light text-sm text-text-main">{c.text}</p>
                                </div>
                            ))}
                        </div>
                        {isAuthenticated ? (
                            <Card className="p-4">
                                ...
                                />
                                <Button variant="gold" size="sm" className="mt-2 font-medium" onClick={handleComment} disabled={commentSubmitting || !commentText.trim()}>
                                    {commentSubmitting ? 'Posting...' : <><Send className="mr-1 w-3 h-3" /> Post Comment</>}
                                </Button>
                            </Card>
                        ) : (
                            <Card className="p-4 text-center">
                                <p className="font-light text-text-main mb-2">Login to join the conversation</p>
                                <Link to="/login"><Button variant="gold" size="sm" className="font-medium">Log In</Button></Link>
                            </Card>
                        )}
                    </div>
                    ...
                    {/* Related Posts */}
                    <div>
                        <h3 className="font-serif font-semibold text-heritage-terracottaDark mb-4">Related Articles</h3>
                        <div className="grid md:grid-cols-3 gap-4">
                            {relatedPosts.map((rp, i) => (
                                <Link key={i} to={`/blog/${rp.slug}`}>
                                    <Card className="p-4 hover:shadow-md transition-shadow">
                                        <Badge variant="secondary" className="text-xs mb-2 font-light">{rp.category}</Badge>
                                        <h4 className="font-serif font-semibold text-sm text-heritage-terracottaDark">{rp.title}</h4>
                                    </Card>
                                </Link>
                            ))}
                        </div>
